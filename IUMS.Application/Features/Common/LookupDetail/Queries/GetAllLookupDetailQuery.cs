    ...
    : IRequestHandler<GetAllLookupDetailQuery, Result<IEnumerable<LookupDetailResponse>>>
{
    public async Task<Result<IEnumerable<LookupDetailResponse>>> Handle(GetAllLookupDetailQuery request, CancellationToken cancellationToken)
    {
        try
        {
            var sql = "SELECT *, cl.Name AS LookupName, cl.NameBN AS LookupNameBN, MD1.Name AS ParentsName FROM dbo.Com_LookupDetails AS MD LEFT JOIN dbo.Com_LookupDetails AS MD1 ON MD.ParentId = MD1.Id LEFT JOIN dbo.Com_Lookups AS cl ON MD.LookupId = cl.Id ORDER BY MD.CreatedOn DESC";
